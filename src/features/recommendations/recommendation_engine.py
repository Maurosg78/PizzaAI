...
from src.core.services.usda_service import USDAService, FoodItem, NutrientInfo
...
class RecommendationEngine:
    ...
    async def get_recommendations(
        ...
        for food in potential_foods:
            ...
            if score > 0:
                ...
                scored_foods.append(Recommendation(
                    food_item=food,
                    score=score,
                    reasons=reasons,
    ...
    async def _get_potential_foods(self, user_prefs: UserPreferences) -> List[FoodItem]:
        ...
        for food in all_foods:
            if not any(restriction in food.description.lower() 
    ...
    def _calculate_nutritional_score(
        ...
        for nutrient, target in nutritional_goals.items():
            nutrient_value = next(
                (n.amount for n in food.nutrients if n.name.lower() == nutrient.lower()),
                0.0
            )
    ...
    def _calculate_balance_score(
        ...
        new_profile = self.nutrition_analyzer.analyze_nutritional_profile(
            current_diet + [food]
        )
    ...
    def _generate_recommendation_reasons(self, food: FoodItem, score: float) -> List[str]:
        ...
        for nutrient, name in main_nutrients.items():
            value = next(
                (n.amount for n in food.nutrients if n.name.lower() == nutrient),
                None
            )
