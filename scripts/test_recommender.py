...
def test_recommender():
    ...
    try:
        ...
        for ingredient in ingredients:
            ...
            if recommendations:
                logger.info(
                    f"Recomendaciones encontradas para {ingredient} (tiempo: {elapsed_time:.2f}s):"
                )
                for rec in recommendations:
                    ...
                    for nutrient in rec["nutrients"]:
                        if nutrient.get("nutrient", {}).get("name") in key_nutrients:
                            logger.info(
                                f"  {nutrient.get('nutrient', {}).get('name')}: {nutrient.get('amount')} {nutrient.get('nutrient', {}).get('unitName')}"
                            )
            else:
