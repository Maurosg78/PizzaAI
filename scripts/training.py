...
class Masa:
    ...
    def validate_formulation(self):
        ...
        if abs(total - 1.0) > 0.01:
            raise ValueError(f"La suma de proporciones en {self.name} debe ser aproximadamente 1.0 (es {total})")
    ...
    def predict_properties(self):
        ...
        calories = sum(ratio * nutritional_data.get(ing, {'calories': 0})['calories']
                       for ing, ratio in self.formulation.items())
        protein = sum(ratio * nutritional_data.get(ing, {'protein': 0})['protein']
                      for ing, ratio in self.formulation.items())

        return {'elasticity': elasticity, 'density': density, 'calories': calories, 'protein': protein}

# Formulaciones iniciales basadas en Greensy
formulations = {
    'C12': Masa('C12', 'cauliflower', {
        'cauliflower': 0.323, 'rice_flour': 0.323, 'maize_flour': 0.162,
        'corn_starch': 0.041, 'egg_substitute': 0.02, 'salt': 0.008, 'water': 0.123
    }),
    'G12': Masa('G12', 'chickpea_flour', {
        'chickpea_flour': 0.204, 'rice_flour': 0.122, 'potato_flour': 0.143,
        'corn_starch': 0.163, 'olive_oil': 0.061, 'salt': 0.01, 'water': 0.244,
        'xanthan_gum': 0.008, 'sugar': 0.045
    })
}
...
def optimize_recipe(masa_name, target='min_density', min_elasticity=0.5, max_calories=300):
    ...
    for ratio in np.arange(0.1, 0.4, 0.05):
        ...
        if result['elasticity'] >= min_elasticity and result['calories'] <= max_calories:
            ...
            if (target == 'min_density' and score < best_score) or (target == 'min_calories' and score > best_score):
                best_score = score
                best_result = result
